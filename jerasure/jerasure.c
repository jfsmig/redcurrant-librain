...
int jerasure_make_decoding_matrix(int k, int w, int *matrix, int *erased, int *decoding_matrix, int *dm_ids)
{
  int i, j, *tmpmat;
...
/* Internal Routine */
int jerasure_make_decoding_bitmatrix(int k, int w, int *matrix, int *erased, int *decoding_matrix, int *dm_ids)
{
  int i, j, *tmpmat;
  int index, mindex;
                          ...
                          char **data_ptrs, char **coding_ptrs, int size)
{
  ...
  if (edd > 1 || (edd > 0 && (!row_k_ones || erased[k]))) {
    ...
    if (jerasure_make_decoding_matrix(k, w, matrix, erased, decoding_matrix, dm_ids) < 0) {
      free(erased);
      free(dm_ids);
                            ...
                            char **data_ptrs, char **coding_ptrs, int size, int packetsize)
{
  ...
  if (edd > 1 || (edd > 0 && (row_k_ones != 1 || erased[k]))) {
    ...
    if (jerasure_make_decoding_bitmatrix(k, w, bitmatrix, erased, decoding_matrix, dm_ids) < 0) {
      free(erased);
      free(dm_ids);
      free(decoding_matrix);
      return -1;
