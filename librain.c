...
int
rain_rehydrate_noalloc(uint8_t **data, uint8_t **coding,
		...
		struct rain_encoding_s *enc, struct rain_env_s *env)
{
	...
	int res = rain_rehydrate_noalloc(data, coding, enc, erasures);
...
int
rain_encode_noalloc (uint8_t **data, struct rain_encoding_s *encoding,
		uint8_t **parity)
{
	assert(encoding != NULL);
		...
		uint8_t **out)
{
	...
	int res = rain_encode_noalloc(data, encoding, parity);
